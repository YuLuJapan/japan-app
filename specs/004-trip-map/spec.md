...
- Flag behaviour follows the app's existing rule: the default applies when no answer has arrived, so the map is invisible on a device's first run until flags load. Shipping off and flipping the default later, keeping the flag as a kill switch, is a rollout decision for the plan, not a requirement here.
