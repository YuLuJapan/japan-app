...
- [ ] T073 Decide the rollout with the user: ship with `show-map` off and flip the code default to `true` after verifying on the trip, keeping the flag as a kill switch — or leave the default off permanently. Note the consequence either way: with the default off, the tab is absent on a device's first run until PostHog answers and never appears if that request fails.
