...
- [x] T081 Re-read every diff in the feature adversarially against `src/map/engine.types.ts`: confirm nothing outside `src/map/engine.leaflet.ts` imports `leaflet`, and that `src/pages/PlaceForm.tsx` still does not import `src/map/`.
- [x] T082 Confirm each slice reverts cleanly in a scratch branch: revert Slice D, then C, then B, and check the app still builds and its tests pass at each step. **A rollback plan nobody has executed is a hypothesis** (plan → Quick rollback). _Executed: D → C → B on a scratch branch; typecheck and the whole suite green at each step (1029, then 1003, then 982 tests), and the entry chunk back at exactly its 233.71 KB gzip baseline with the map gone._
...
- [x] T084 Final gate — `npm test && npm run typecheck && npm run lint && npm run format` — and confirm no debug flag flips (`useBooleanFlag('show-map', true)`) survived from local development.
