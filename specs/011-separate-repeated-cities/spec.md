...
- **A travel day belongs to two stops at once.** Stop dates overlap by a day at every handover — on the Japan trip, all 9 of them (Tokyo ends 25 Sep, Hakone begins 25 Sep). So "the visit whose dates contain this day" is ambiguous once per handover, and both the division of existing content and the day-plan link depend on that phrase. The app **already has an answer** — the city you sleep in that night, which is what decides where a newly-added activity is filed — and this feature must reuse it rather than adopt a second one. Two visits of the *same* city placed back to back is the case that answer was never asked about, and the only one this feature adds.
...
- **FR-015a**: Where a day falls inside two visits because one stop's last day is the next stop's first — every handover on the Japan trip — the day MUST resolve to the visit the traveller **sleeps in that night**, and on the trip's last day to the final stay. This is **not a new rule**: it is the definition the app already applies when deciding which city a newly-added activity belongs to. Both the day plan and the division of existing content (FR-012) MUST use that one definition rather than inventing a second, and where it is necessarily written twice (once per runtime) the two copies MUST be tested against each other over the same rows.
...
- **The day plan's dates are trustworthy** as a signal of which visit an activity belongs to, since every stop's dates sit inside the trip's dates. Stops *do* overlap by one day at each handover, which is why FR-015a exists; beyond that single shared day a date names exactly one visit. FR-015a deliberately adopts the app's existing answer rather than a better one — a second definition of "which city is this day" would be worse than an imperfect single one.
