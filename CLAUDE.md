...
**Auth:** shared bearer access codes, not per-user auth. `authMiddleware` (`server/src/lib/auth.ts`) exempts only `/api/health` and `/api/auth/verify`. Frontend stores the code in `localStorage` (`src/api/client.ts`); a 401 anywhere clears it and redirects to `/gate`. This is a convenience lock, not real security (documented in README) — don't add stronger auth machinery unless asked.

Two codes map to two roles, decided by which one is sent: `TRIP_ACCESS_CODE` → `owner` (the travelers, full access), `TRIP_GUEST_CODE` → `guest` (friends). Guests are stopped **in the middleware, before any route runs**: non-`GET` → `403 FORBIDDEN`, anything under `/api/files` → `403` (reads included), and `getZoneDetail`/`getPlaceDetail` are called with `includeFiles: false` so attachments never reach them. The frontend mirrors this with `useCanEdit()` (`src/lib/session.tsx`, fed by `GET /api/auth/session`) and a `RequireOwner` route guard — but that is only there to avoid offering buttons that would fail. **Any new write endpoint is guest-proof automatically by virtue of its HTTP method; any new route that serves file bytes outside `/api/files` is not** — gate it explicitly.

Guests are also kept from the **bookings that aren't files** (`server/src/lib/guest-view.ts`): the stays (a `hotel` place carries the reservation — price, confirmation, cancellation terms, the Booking.com link — in its free-text description, so the whole category is withheld rather than redacting prose) and the `flight` block. These are plain `GET`s, so the middleware can't do it; each read path passes `includeStays`/`includeFlight` from `!isGuest(req)` — place detail (403 on a stay), zone detail and zone place lists, the trip bundle, search, and the itinerary (a stay's `place_id` is nulled so the day plan doesn't link into a 403). **Anything new that returns a place, a place id, or trip-level booking metadata needs the same flag** — a new route inherits nothing here.
