...
- `accounts.ts` — five real Auth accounts on fixed UUIDs (owner, partner, viewer, outsider, unconfirmed). Tokens come from GoTrue's password grant and are verified by the real `lib/identity.ts`. They are shared by the whole run, so **a test that signs one of them out breaks every test after it**: `signOut()` revokes the account's sessions, and the provisioned token then 401s. A file that signs in and out for real makes its own account (`createAccount()` in `src/tests/data.ts`) and deletes it afterwards.
...
**A test run reaches neither the live project nor the internet.** Third parties may be substituted — that is what the fixture server is — but nothing is allowed to become a real request by accident: `blockOutboundFetch`/`blockOutboundNodeHttp` (`server/testing/no-outbound.ts`) wrap `fetch` and `http(s).request` in every process, pass loopback straight through to the real implementation, and refuse anything else. Refusals are _recorded_ as well as rejected, because several services read a failed fetch as "nothing found" and would otherwise swallow the evidence; the setup files fail the test that caused one. The API runs in the globalSetup process, so its attempts come back over a control route on the fixture server. This is how the one real leak was found — a shopping test used an unresolvable hostname, which still asks a DNS server; it now points at a fixture route that drops the connection.
