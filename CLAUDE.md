...
**Roles decide verbs, three flags decide content.** `owner` > `partner` > `viewer` in `lib/permissions.ts` (pure, table-tested). Writes are refused once, in `requireTripAccess`, for any role that can't write — the sole exception being a member removing _themselves_. A trip that isn't yours answers **404, never 403**; a member who merely lacks the verb gets 403, since they already know it exists.

Content is `lib/trip-view.ts`: `can_see_stays` / `can_see_flight` / `can_see_documents` on the member row collapse into one `TripView` on the context. Writers always get the full view — the flags are _ignored_, not validated, so an owner can't lock themselves out. A `hotel` place carries the reservation in free-text description and links, so the whole category is withheld rather than redacted; the `flight` block is the same story in structured form. Enforcement points: place detail (403 on a hidden stay), zone detail and its counts, zone place lists, the trip bundle's `flight`, search, files, and the itinerary (a hidden stay's `place_id` is nulled so the day plan doesn't link into a 403). **Anything new that returns a place, a place id, a file or trip-level booking metadata needs the same treatment** — a new route inherits the access check, not the view.
