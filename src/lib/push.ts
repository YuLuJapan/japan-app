 ...
 * dot on the Reminders tab. Backed by the notification tray itself rather
 * than separate storage, so it can never drift from what's actually showing.
 * Every notification this app shows is reminder-related (a real reminder
 * tagged `reminder-<id>`, or a `test` push), so no tag filter is needed —
 * and `getNotifications` only matches an *exact* tag anyway, which a
 * per-reminder tag never would.
 */
